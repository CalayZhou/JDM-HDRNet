...
class EncoderDecoder(BaseSegmentor):
    ...
    def extract_feat_2(self, img_2, **kwargs):
        ...
        # while img_2.dim() > 4:
        #     img_2 = img_2.squeeze(0)
        # img_2 = img_2.to(torch.float32)
        # return img_2
    ...
    def encode_decode(self, img, img_2, img_metas, **kwargs):
        ...
        修改了这个函数，加入了对第二路输入的处理
        """
        # fusion at input
        # img_2 = img_2.unsqueeze(0)
        # img_2 = img_2.to(torch.float32)
        # if img.shape[2:] != img_2.shape[2:]:
        #     img_2 = F.interpolate(img_2, size=img.shape[2:], mode='nearest')
        #
        # # 将两路进行拼接
        ...
        # img_2 = img_2.permute(0, 3, 1, 2)
        # spatio_scale = 16
        # img_2= resize(img_2, (spatio_scale, spatio_scale), Image.BILINEAR)
        # img_2 = img_2.to(torch.float32)
        # img_2 = img_2.permute(0, 3, 1, 2)
        # print('encode decode img', img.shape)
        # print('encode decode img_2', img_2.shape)
        # img_2 = img_2.to(torch.float32)/65535.0
        # spatio_scale = 16
        # img_2 = resize_tensor(img_2, (spatio_scale, spatio_scale), Image.BILINEAR)
        # img_2 = img_2.permute(0, 2, 3, 1).contiguous()
        '''
        img_2 = img_2.to(torch.float32)/65535.0
        ...
        # spec_low = resize(spec_tmp, (self.input_res, self.input_res), Image.NEAREST)
        ...
        x_2 = list(x_2)
        layer_i = -1
        # # 检查第一路和第二路的H W维度是否一致，如果不一致则进行调整
        # if x[layer_i].shape[2:] != x_2[layer_i].shape[2:]:
        #     x_2[layer_i] = F.interpolate(x_2[layer_i], size=x[layer_i].shape[2:], mode='bilinear', align_corners=True)
        # 将两路特征进行拼接
